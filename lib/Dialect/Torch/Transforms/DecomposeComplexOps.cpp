...
public:
  ...
  LogicalResult matchAndRewrite(AtenAmaxOp op,
    ...
    for (int64_t dimInt : dims) {
      int64_t inputRank = input.getType().cast<RankedTensorType>().getRank();
      dimInt = toPositiveDim(dimInt, inputRank);
      if (!isValidDim(dimInt, inputRank))
...
public:
  ...
  LogicalResult matchAndRewrite(AtenScatterValueOp op,
    ...
    Type srcType = selfType.getWithSizesAndDtype(
        llvm::makeArrayRef(indexType.getSizes()), selfType.getDtype());
    Value src =
